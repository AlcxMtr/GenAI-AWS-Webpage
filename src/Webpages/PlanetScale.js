...
const Feature = ({ title, description, neg }) => {
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener('resize', updateWidth); // Cleanup listener on unmount
    };
  }, [title]); // Add title as a dependency

  return (
    <div className="flex flex-col items-start">
    <div ref={titleRef} className="text-lg font-semibold mb-1">
      {title}
    </div>
    <div
      className={`relative transform ${neg}rotate-6 bg-gray-800 h-0.5 my-4`}
...
const MainContent = () => (
  <div className="relative grid grid-rows-4 grid-cols-5 gap-6 p-4 h-full">
      ...
      <div className="col-span-2 row-span-1">
        <div className="bg-black w-full h-full rounded-3xl flex flex-row items-center"
          ...
          <button className={`bg-purple-300 rounded-full m-2 mr-6 h-12`}>
      ...
      <div className="absolute top-0 left-0 w-full h-full grid grid-rows-4 grid-cols-5 pointer-events-none">
        <div className="row-span-3 col-span-3">
          <h1 className="text-6xl font-bold mt-10 ml-10 text-left">Payment processing for complex needs</h1>
        </div>
      </div>
    </div>
);
